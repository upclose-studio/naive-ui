...
import type { DatePickerThemeVars } from '../styles/self'
...
export type DatePickerPanelMap = Partial<Record<DatePickerType, Component>>

export interface DatePickerFactoryOptions {
  name: string
  theme: Theme<'DatePicker', DatePickerThemeVars>
  panels: DatePickerPanelMap
  defaultType?: DatePickerType
  allowedTypes?: readonly DatePickerType[]
  props?: typeof datePickerProps | Record<string, unknown>
}
...
export function createDatePicker(options: DatePickerFactoryOptions) {
  const pickerProps = (options.props ?? {
    ...datePickerProps,
    type: {
      ...datePickerProps.type,
      default: options.defaultType ?? 'date'
    }
  }) as typeof datePickerProps
  ...
  return defineComponent({
      ...
      if (__DEV__) {
        watchEffect(() => {
          ...
          if (
            ...
            && !options.allowedTypes.includes(props.type)
          ) {
            warnOnce(
              options.name,
              `type "${props.type}" is not supported. Use NDatePicker.`
            )
          }
        })
      }
    ...
    render() {
      ...
      const renderPanel = (): VNode | null => {
        const { type } = this
        const Panel = options.panels[type] as any
        if (!Panel) {
          return null
        }
        return type === 'datetime' ? (
          ...
          <Panel {...commonPanelProps} type={type} key={type} />
        ) : type === 'monthrange'
          || type === 'yearrange'
          || type === 'quarterrange' ? (
              <Panel {...commonPanelProps} type={type} />
            ) : (
              <Panel
                {...commonPanelProps}
                type={type}
